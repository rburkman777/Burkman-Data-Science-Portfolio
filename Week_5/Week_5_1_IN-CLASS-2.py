...
# Provide opions fo rhow to handle missing data

method = st.radio("Choose a method",
         ["Original DF", "Drop Rows", "Drop Columns (>50% Missing)",
          "Impute Mean", "Impute Median", "Impute"])
...
if method == "Original DF":
    ...
    #Remove all rows that contain any missing vlaues
    ...
    #Use fillna to impute zero of column

    df_clean[column==0] = df_clean[column==0].fillna(0)
...
#
# Display side-by-side histograms and statistical summaries for the selected
#column
# -------------
